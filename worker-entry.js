...
async function handleRefresh(env) {
  ...
  if (!ANTHROPIC_API_KEY || !GH_TOKEN) {
    return new Response(JSON.stringify({ error: 'env_vars_missing', message: 'ANTHROPIC_API_KEY と GH_TOKEN を環境変数に設定してください' }), {
...
以下のJSON形式で出力（JSON以外は出力しないこと）:
{
  ...
  "yoshinoReading": "吉野敏明視点での読み解き。断定的な医療効果の表現は避けエビデンスベースで記述",
...
重要: 医師法に違反しない表現。架空のURLは使わない。`
    ...
    return new Response(JSON.stringify({ status: 'ok', newArticle }), {
